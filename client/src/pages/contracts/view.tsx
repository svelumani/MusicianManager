          ...
          <div className="p-6 border rounded-md bg-white">
            ...
            <div className="grid grid-cols-2 gap-8 mb-6">
              ...
              <div>
                ...
                <p>{musician?.name || "Loading..."}</p>
                <p>{musician?.email || "Loading..."}</p>
                <p>{musician?.phone || "Loading..."}</p>
              </div>
            </div>
            
            <h3 className="font-bold mb-2">Event Details</h3>
            <p className="mb-1"><strong>Event Name:</strong> {event?.name || "Loading..."}</p>
            <p className="mb-1"><strong>Location:</strong> {event ? `Venue #${event.venueId}` : "Loading..."}</p>
            {contract.eventDate && (
...
function Input({ value, readOnly, className }: { value: string, readOnly: boolean, className?: string }) {
  return (
    <input
      ...
      value={value}
      readOnly={readOnly}
      className={`w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm ${className}`}
    />
  );
}
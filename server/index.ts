...
// Enable JSON body parsing globally before any routes
...
// Direct endpoint for accepting contracts by token
app.post("/api/v2/contracts/token/:token/accept", async (req, res) => {
  try {
    const token = req.params.token;
    const { signature, response } = req.body;
    
    log(`[PRE-MIDDLEWARE API] Contract acceptance request for token: ${token}, signature: ${signature ? 'provided' : 'missing'}`);
    
    console.log(`[ACCEPT] Contract acceptance payload:`, req.body);
    ...
    if (!signature) {
      log(`[ACCEPT] Contract acceptance rejected - no signature provided for token: ${token}`);
    ...
    if (contractResult.rows.length === 0) {
      log(`[ACCEPT] Contract not found with token: ${token}`);
      return res.status(404).json({ 
        message: "Contract not found", 
        token,
        timestamp: new Date().toISOString()
    ...
    const contract = contractResult.rows[0];
    log(`[ACCEPT] Found contract: #${contract.id} for musician: ${contract.musician_id}, event: ${contract.event_id}`);
    
    // Check if contract has already been responded to
    if (contract.responded_at) {
      log(`[ACCEPT] Contract #${contract.id} already responded to at ${contract.responded_at}`);
      return res.status(400).json({
    ...
    if (new Date() > expiresAt) {
      log(`[ACCEPT] Contract #${contract.id} has expired at ${contract.expires_at}`);
      return res.status(400).json({
    ...
    log(`[ACCEPT] Updating contract #${contract.id} status to contract-signed with signature: ${signature}`);
    ...
    if (updateResult.rows.length === 0) {
      log(`[ACCEPT] Failed to update contract #${contract.id}`);
    ...
    return res.json({
      ...
      contract: {
        ...
        eventDate: updatedContract.event_date,
    ...
    console.error('[ACCEPT] Contract acceptance error:', error);
    res.status(500).json({ 
      error: "Internal server error", 
      message: String(error),
      timestamp: new Date().toISOString()
...
// Direct endpoint for accepting a contract by token
app.post("/api/v2/contracts/token/:token/accept", async (req, res) => {
  try {
    const token = req.params.token;
    const { signature, response } = req.body;
    ...
    console.log(`[DEBUG] Contract acceptance payload:`, req.body);
    ...
    const updateResult = await pool.query(`
      ...
      WHERE token = $4
      RETURNING *
    `, [response || 'Contract accepted', now, signature, token]);
    ...
    return res.json({
      ...
      contract: {
        ...
        respondedAt: updatedContract.responded_at,
        response: updatedContract.response,
        signature: updatedContract.musician_signature
      },
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    log(`[PRE-MIDDLEWARE API] Contract accept error: ${error}`);
    console.error('[ERROR] Contract acceptance error:', error);
    res.status(500).json({ 
      error: "Internal server error", 
      message: String(error),
      timestamp: new Date().toISOString()
...
// Middleware to capture API requests for logging
